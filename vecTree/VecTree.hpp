...
class VecTree {
  struct Node {
    ...
    Node(int parent, const T& element_data)
        : parent{parent}, data{element_data}, left{-1}, right{-1} {}
...
int VecTree<T>::foundIndex(int index, const T& element_data) const {
  if (index == -1)
    ...
    if (children[index].left != -1 &&
        children[index].left > static_cast<int>(children.size() - 1))
      throw std::out_of_range(
          "******Index in foundIndex() out of range!!!!******");
    return foundIndex(children[index].left, element_data);
  } else {
    if (children[index].right != -1 &&
        children[index].right > static_cast<int>(children.size() - 1))
      throw std::out_of_range(
          "******Index in foundIndex() out of range!!!!******");
...
void VecTree<T>::erase(const T& element_data) {
  ...
  if (children[toBeRemoved].data != element_data)
    throw std::runtime_error(
        "**********foundIndex() has provided an incorrect index!!!!**********");
  ...
  auto setParentChild = [&](int* PARENT_HAND, int PARENT, int CHILD) {
    if ((*PARENT_HAND = CHILD) != -1) children[CHILD].parent = PARENT;
  };
  ...
  auto deleteElement = [&]() {
    if (toBeRemoved !=
        lastIndex) {  // Move vec.back() to element being removed and pop_back()
                      // the vec.back() element that is now empty
  ...
  if (children[current].right == -1 && children[current].left == -1) {
    if (head != toBeRemoved) leftOrRight();
    deleteElement();
    return;
    ...
    if (children[children[current].right].left == -1) {
      ...
      setParentChild(&children[replacingWith].left, replacingWith,
                     children[toBeRemoved].left);
    } else {
      current = children[current].right;
      while (children[children[current].left].left != -1)
        current = children[current].left;
      replacingWith = children[current].left;
      setParentChild(&children[current].left, current,
                     children[replacingWith].right);
      setParentChild(&children[replacingWith].left, replacingWith,
                     children[toBeRemoved].left);
      setParentChild(&children[replacingWith].right, replacingWith,
                     children[toBeRemoved].right);
    }
  }
  if (head != toBeRemoved)
    leftOrRight();
...
void VecTree<T>::traverse(int index, VecTree<T>::Order order) const {
  ...
  auto printNodeValues = [&]() {
    ...
    if (children[index].left != -1)
      std::cout << children[children[index].left].data << ", ";
    else
      std::cout << "NULL"
                << ", ";
    if (children[index].right != -1)
      std::cout << children[children[index].right].data << std::endl;
    else
      std::cout << "NULL"
                << ", " << std::endl;
  };
